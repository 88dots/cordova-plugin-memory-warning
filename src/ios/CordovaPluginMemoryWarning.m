...
- (void)checkMemoryUsage:(CDVInvokedUrlCommand*)command
{
    ...
    CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsBoolean:false];
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}

- (void)onMemoryWarning
{
    NSString *jsCommand = [@"cordova.fireWindowEvent('memorywarning');"];
